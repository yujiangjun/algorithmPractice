 ...
 */
//--------------测试未通过-----------------
public class Code7_xtoYminDistance {
    ...
    private static int process(int cur, int aim, int n, int[][] map, boolean[] visited) {
        if (visited[cur]){
        ...
        for (int i = 1; i < n; i++) {
            if (i!=cur&&map[cur][i]!=Integer.MAX_VALUE){
                int rest=process(i,aim,n,map,visited);
                if (rest!=Integer.MAX_VALUE){
                    ans=Math.min(ans,map[cur][i]+rest);
                }
            }
        }
        visited[cur]=false;
    ...
    public static int minDistance2(int n,int m,int[][] roads,int x,int y){
        ...
        while (!heap.isEmpty()){
            ...
            for (int i = 1; i <= n; i++) {
                if (i!= cur.city&&map[cur.city][i]!=Integer.MAX_VALUE&&!computed[i]){
                    heap.add(new Node(i, cur.pathSum+map[cur.city][i]));
                }
            }
        }
        return Integer.MAX_VALUE;
